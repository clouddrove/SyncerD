...
updateJson("SyncerD/task.json", (data) => {
  data.version = { Major: major, Minor: minor, Patch: patch };
});

updateJson("SyncerD/package.json", (data) => {
  data.version = version;
});
NODE
...
echo "Preparing SyncerD Azure DevOps extension $NEXT_VERSION"
...
echo "Published clouddrove.syncerd-azure-pipelines $NEXT_VERSION"
echo "Check validation:"
echo "tfx extension isvalid --publisher clouddrove --extension-id syncerd-azure-pipelines --version $NEXT_VERSION --service-url https://marketplace.visualstudio.com/ --token '<PAT>'"
