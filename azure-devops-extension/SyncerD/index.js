...
async function installSyncerD(versionInput) {
  ...
  const fileName = `syncerd_${assetVersion}_${releaseOS}_${releaseArch}.${extension}`;
  const url = `https://github.com/${REPO}/releases/download/${version}/${fileName}`;
  const baseTemp = task.getVariable("Agent.TempDirectory") || os.tmpdir();
  const workDir = fs.mkdtempSync(path.join(baseTemp, "syncerd-"));
  const archive = path.join(workDir, fileName);
  const installDir = path.join(workDir, "bin");

  console.log(`Installing syncerd ${version} for ${releaseOS}/${releaseArch}`);
  ...
  const binaryName = isWindows ? "syncerd.exe" : "syncerd";
...
function getText(url) {
  return new Promise((resolve, reject) => {
    https
      .get(url, { headers: { "User-Agent": "syncerd-azure-pipelines-task" } }, (response) => {
...
async function run() {
  try {
    ...
    const syncerdPath = await installSyncerD(version);

    if (!command.trim()) {
      console.log("syncerd installed successfully.");
      return;
    }

    const args = parseCommand(command);
    const runner = task.tool(syncerdPath);
    runner.arg(args);
